...
public class DgpuFreeUpNowTests
{
    ...
    private static DgpuApp Closable(int pid, string name = "blender") =>
        new(pid, name, 500L * 1024 * 1024, DgpuAppVerdict.Close, Started);

    private sealed class Rig : IDisposable
    {
        public Rig(bool pluggedIn)
        {
            Power = new FakePowerSource(pluggedIn);
            Coordinator = new DgpuUnplugCoordinator(
                Power,
                () => Task.FromResult(NextScan()),
                async (_, dismissWhenPluggedIn) =>
                {
        ...
        public Func<DgpuScanResult> NextScan { get; set; } = () => new DgpuScanResult(true, false, [Closable(10)]);
    ...
    {
        using var rig = new Rig(pluggedIn: true);
    ...
    {
        using var coordinator = new DgpuUnplugCoordinator(
